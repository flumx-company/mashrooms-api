...
export class InitialSchema1706543218570 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`prices\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`tenant\` enum ('BARREL', 'BOX', 'KITCHEN') NOT NULL DEFAULT 'BARREL', \`price\` decimal(10,2) NOT NULL DEFAULT '0.00', \`startTime\` date NOT NULL, \`endTime\` date NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
